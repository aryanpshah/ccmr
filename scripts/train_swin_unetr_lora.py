...
def main():
    ...
    parser.add_argument("--lr_lora", type=float, default=3e-4, help="Learning rate for LoRA/decoder/head params. CRITICAL FIX: Reduced from 5e-4 to 3e-4")
    parser.add_argument("--lr_backbone", type=float, default=5e-6, help="Learning rate for backbone params. CRITICAL FIX: Reduced from 1e-5 to 5e-6")
    parser.add_argument("--wd_lora", type=float, default=3e-2, help="Weight decay for LoRA/decoder/head params. CRITICAL FIX: Increased from 1e-2 to 3e-2")
    parser.add_argument("--wd_backbone", type=float, default=1e-3, help="Weight decay for backbone params. CRITICAL FIX: Increased from 1e-4 to 1e-3")
    parser.add_argument("--warmup_epochs", type=int, default=15, help="Linear warmup epochs before decay. CRITICAL FIX: Increased from 5 to 15")
